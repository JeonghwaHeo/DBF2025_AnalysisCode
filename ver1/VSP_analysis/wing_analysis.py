...
horizontal_AR = 4
horizontal_taper = 1
horizontal_ThickChord = 8 # 왜 input으로..?
...
vsp.SetParmVal(wing_id, "Twist", "XSec_0", 2)
...
vsp.SetParmVal(wing_id, "Density", "Mass_Props", wing_density)
vsp.Update()
...
vsp.SetParmVal(tailwing_id, "Density", "Mass_Props", wing_density)
vsp.Update()
...
vsp.SetParmVal(verwing_right_id, "Density", "Mass_Props", wing_density)
vsp.Update()
...
vsp.SetParmVal(verwing_left_id, "Density", "Mass_Props", wing_density) 
vsp.Update()
...
def calculate_coefficient(vsp_file, alpha_start, alpha_end, alpha_step, flap_angle, Re, Mach):
    ...
    wing_c_root = vsp.GetParmVal(vsp.GetParm(wing_id,"Root_Chord","XSec_1"))
    tail_c_root = vsp.GetParmVal(vsp.GetParm(tailwing_id,"Root_Chord","XSec_1"))
...
"""
여기서부턴 그래프 그리는 코드, main에 합칠땐 반영하지 않아도 됨됨
"""
